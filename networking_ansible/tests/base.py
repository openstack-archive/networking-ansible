...
class NetworkingAnsibleTestCase(test_plugin.Ml2PluginV2TestCase):
    def setUp(self):
        ...
        self.settings = {'pexpect_use_poll': False}
    ...
    def gen_playbook(self, task, host, segid, switchport=None):
        segname = 'default' if segid == '1' else 'vlan{}'.format(segid)
        data = {'name': 'Openstack networking-ansible playbook',
             'gather_facts': 'no',
             'tasks': [
                 {'name': 'do {}'.format(task),
                  'vars': {
                  ...
                  'import_role': {'name': 'openstack-ml2',
                                  'tasks_from': task,}}],
             'hosts': host
            }
        if switchport:
            data['tasks'][0]['vars']['port_description'] = switchport
            data['tasks'][0]['vars']['port_name'] = switchport
        return [ data ]
